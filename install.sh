...
if [ "$PLATFORM" = "macos" ]; then
    ASSET_NAME="emailgen-macos-${ARCH}.tar.gz"
    ...
    ASSET_NAME="emailgen-linux-x86_64.tar.gz"
elif [ "$PLATFORM" = "windows" ]; then
    ASSET_NAME="emailgen-windows-x86_64.zip"
fi
...
echo "Downloading emailgen ${LATEST_RELEASE} for ${PLATFORM}-${ARCH}..."
TEMP_DIR=$(mktemp -d)
...
if [ "$PLATFORM" = "windows" ]; then
    ...
    mv "${TEMP_DIR}/emailgen.exe" "${INSTALL_DIR}/"
else
    # Extraction for Linux/macOS
    tar -xzf "${TEMP_DIR}/${ASSET_NAME}" -C "${TEMP_DIR}"
    mv "${TEMP_DIR}/emailgen" "${INSTALL_DIR}/"
    chmod +x "${INSTALL_DIR}/emailgen"
fi

rm -rf "${TEMP_DIR}"

echo "Successfully installed emailgen ${LATEST_RELEASE}!"
emailgen --version
