...
# Build if needed
if [ ! -f "target/release/mailgen" ]; then
    echo "Building mailgen..."
...
if [ -n "$OUTPUT" ]; then
    ...
    ./target/release/mailgen --count $COUNT --output "$OUTPUT" --progress
    echo "Done! Generated $COUNT emails."
    echo "File size: $(ls -lh "$OUTPUT" | awk '{print $5}')"
else
    ./target/release/mailgen --count $COUNT --quiet
fi
